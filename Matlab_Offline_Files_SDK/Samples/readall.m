...
%addpath('ReadingPLXandDDTfiles/')
addpath('/snel/home/fzhu23/bin/analysis_tools/Matlab_Offline_Files_SDK/');
% Open a plx file
% this will bring up the file-open dialog
%StartingFileName = '/mnt/scratch/feng/RP_06182018_SPKC.pl2';
%StartingFileName = '/mnt/scratch/feng/Remy_RP_02052019_SPKC_E2.pl2';
StartingFileName = '/mnt/scratch/feng/Remy_02262019_PUL_Raw.pl2';
...
if ( nslowchannels > 0 )
    ...
    for ich = 0:nslowchannels-1
        if ( slowcounts(ich+1) > 0 )
            disp(sprintf('loading channel %g / %g', ich+1, nslowchannels) );
            tic;
            [adfreq, nad, tsad, fnad, allad{ich+1}] = plx_ad(OpenedFileName, ich);
            %[adfreq, n, allad{ich+1}] = ...
                plx_ad_span(OpenedFileName, ich, 1, 1e6);
            numads = numads + 1;
            toc;
        end
    end
...
% what channels have anything loaded?
