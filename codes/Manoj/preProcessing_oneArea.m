...
%for day = 3
    ...
    %rawDataPath = '/snel/share/share/data/kastner/Manoj/PUL/Remy_0208_PUL/';
    %rawDataPath = '/snel/share/share/data/kastner/Manoj/PUL/Remy_0310_PUL/';
    folderName = ['Remy_' datasets(day).shortName '_PUL'];
    fileName = ['Remy_' datasets(day).longName '_PUL_MUA.mat'];

    %dayFileName = 'Remy_02082019_PUL_MUA.mat';
    %dayFileName = 'Remy_03102019_PUL_MUA.mat';
...
trial_selected = [30, 66];
cmap = lines();
legend_cell = {'fix square', 'endo - TL', 'endo - BL', 'endo - TR', 'endo - BR', 'bar - vert', 'bar - hori', 'exo - TL', 'exo - BL', 'exo - TR', 'exo - BR', 'target - TL', 'target - BL', 'target - TR', 'target - BR'};
cue_cell = {'TL', 'BL', 'TR', 'BR'};
for i = 1:length(trial_selected)
    itrial = trial_selected(i);
    ei = r.r(itrial).externalInputs;
    x_bar = UE.barOn(itrial) - UE.fixOn(itrial);
    x_cue = UE.cueOn(itrial) - UE.fixOn(itrial);
    x_cueEnd = UE.cueEnd(itrial) - UE.fixOn(itrial);
    x_target = UE.targetOn(itrial) - UE.fixOn(itrial);
    x_targetEnd = UE.targetEnd(itrial) - UE.fixOn(itrial);
    x_trialEnd = UE.trialEnd(itrial) - UE.fixOn(itrial);
    nSubPlot = 15;
    f1 = figure;
    for j = 1:15
        subplot(15, 1, j);
        plot(ei(j,:), 'Color', cmap(j, :), 'DisplayName', legend_cell{j});
        if j == 15
            if x_targetEnd == x_trialEnd
                set(gca,'XTick',[0, x_bar, x_cue, x_cueEnd x_target x_trialEnd]);
                set(gca,'XTickLabels',{'fix', 'barOn', 'cueOn', '+150ms','targetOn', 'release'});
            else
                set(gca,'XTick',[0, x_bar, x_cue, x_cueEnd x_target x_targetEnd x_trialEnd]);
                set(gca,'XTickLabels',{'fix', 'barOn', 'cueOn', '+150ms','targetOn', '+150ms', 'release'});
            end
            set(gca, 'FontSize', 7);
        end
        ylim([-1 1]);
    ...
    % get title
    if UE.fixType(itrial) == 1
        exoOrEndo = 'Exo - ';
    else
        exoOrEndo = 'Endo - ';
    end
    if UE.barType(itrial) == 1
        vertOrHori = 'Vert Bars - ';
    else
        vertOrHori = 'Hori Bars - ';
    end
    cuedLocation = [cue_cell{DataPreprocessing.Manoj_getTargetPos(UE.cueType(itrial))}, ' - '];
    if UE.isValidTarget(itrial)
        targetLocation = 'Valid target';
    elseif UE.isSameObjTarget(itrial)
        targetLocation = 'Same obj target';
    elseif UE.isDiffObjTarget(itrial)
        targetLocation = 'Diff obj target';
    end
    suptitle([exoOrEndo, vertOrHori, cuedLocation, targetLocation]);
   
    % save figure
    set(f1, 'Position', [4 4 1897 962]);
    savefigDir = '/snel/share/share/derived/kastner/data_processed/ManojData/singleArea/pulvinar/multi-unit/continuous/withExternalInput_withLag/singleDay/testDay_020819/examples_externalInputs';
    if ~isdir(savefigDir)
        mkdir(savefigDir);
    end    
    cd(savefigDir);
