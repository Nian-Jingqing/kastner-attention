...
computers = [
    ...
    # 'max_processes': 6,
    # 'process_start_cmd': '/snel/home/fzhu23/bin/pbt-hp-opt/pbt_opt/run_lfads_client.sh',
...
svr = Server(name, run_save_path, num_workers=32, steps_to_ready=5,
             max_generations=150, exploit_method='binarytournament', exploit_param=[],
             explore_method='perturb', explore_param=0.8, mode='minimize', force_overwrite=True, percent_change_to_stop=0.0, mongo_server_ip='localhost', port=27017, server_log_path=run_save_path, num_no_best_to_stop=8)
...
# Learning rate
svr.add_hp('learning_rate_init', (0.00001, 0.0008), init_sample_mode=[0.0008],
...
# Coordinated Dropout
svr.add_hp('keep_ratio', (0.1, 0.99), init_sample_mode=[0.5],
           explore_method='perturb', explore_param=0.3, limit_explore=True, explorable=True)
...
svr.add_hp('l2_gen_scale', (1e-4, 1), init_sample_mode='logrand', explore_param = 0.6, explorable=True)
svr.add_hp('l2_ic_enc_scale', [0.0], init_sample_mode=[0.0], explorable=False)

svr.add_hp('l2_con_scale', (5e-5, 0.1), init_sample_mode='logrand',  explorable=True)
...
svr.add_hp('kl_co_weight', (1e-7, 1e-4), init_sample_mode='logrand', explore_param = 0.6, explorable=True)
svr.add_hp('kl_ic_weight', (1e-7, 2e-6), init_sample_mode='logrand', explore_param = 0.3, explorable=True)
...
#svr.add_hp('cd_grad_passthru_prob', (0.0, 0.999), init_sample_mode=[0.001], limit_explore=True,  explore_param=0.3, explorable=True) # for run 180614, for sa
...
svr.add_hp('kl_increase_epochs', [80]) # don't change until implemented # FZ
# svr.add_hp('l2_increase_steps', [500])
svr.add_hp('l2_increase_epochs', [80]) # don't change until implemented # FZ
...
svr.add_hp('temporal_spike_jitter_width', [1]) # FZ changed from 0 to 1 on 20191107
...
PM_param = {'batch_size':100, 'valid_batch_size':4000}
