...
%%
outdir = '/snel/share/share/derived/kastner/LFADS_runs/Manoj/pulvinar/180208/postAnalysis/withExternalInput_20190411/lowD_traj/cueOnset_delay_123/bar1cue1234_plot34_split/';
if ~isdir(outdir)
    mkdir(outdir);
end
...
for nday = 1 : numel( alf )
    trialsToKeep{ nday } = (UE{ nday }.barType == 1 & ismember(UE{ nday }.cueType, [1,2,3,4]));
...
weights = [];
rootPath = '/snel/share/share/derived/kastner/LFADS_runs/Manoj/pulvinar/180208/runs/withExternalInput_20190411/param_kYTEaB/all/pbt_run/g037_w10/model_params';
for nday =1: numel(alf)
    dayFile_W = ['/LFADS_' datasets(nday).shortName '_v2.h5_out_fac_linear_W:0'];
    dayFile_b = ['/LFADS_' datasets(nday).shortName '_v2.h5_out_fac_linear_b:0'];
...
%for nday = [1]
    ...
    for np = 1:2
        ...
        %axis(p(np), [-6   6   -6   6   -6    6]);  % for Manoj, cueOnset 0-300ms dimRed
        axis(p(np), [-3   4   -2   4   -3    3]);  % for Manoj, tc data, cueOnset 0-400ms dimRed
