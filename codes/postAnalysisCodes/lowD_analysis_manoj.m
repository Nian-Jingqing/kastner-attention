...
%% loop through each condition and assign condition ID
for nday = 1 : numel( alf )
    trialsToKeep{ nday } = ismember(UE{nday}.cueType, which_cue);
...
%%
window = round( [100  500] / binsize_rescaled );
whichfieldDimred = 'cueOnset';
whichfieldPlot = 'cueOnset';
newWindow = round( [0  200] / binsize_rescaled );

%% get data concatenated for performing PCA. If do all days
...
%% get data concatenated for performing PCA. If do on certain days
which_days = [2, 5, 6, 9];
timePoints = window(1):window(2);
numBins = numel( timePoints);
numFactors = size( alf{ nday }(1).factors, 1);
totalTrialsToKeep = sum( cellfun( @sum, trialsToKeep(which_days) ) );
allFactors = zeros( numFactors, numBins * totalTrialsToKeep );
%%
...
[pca_proj_mat, pc_data, latent, tsquared, explained] = pca( allFactors', 'NumComponents', 3);
...
%for nday = which_days
for nday = 2
    %sp(s_id) = subplot(1, 4, s_id);
    for itr = 1:numel( trialsToKeepInds{ nday } )
        ntr = trialsToKeepInds{ nday }( itr );
        
        if UE{nday}.barType(ntr) == 1
            continue;
        end
        
        color_thisCond = cmap(UE{nday}.cueType(ntr), :);
        ...
        plot3(dim_reduced_data(1, :), dim_reduced_data(2, :), dim_reduced_data(3, :), 'Color', color_thisCond, 'LineWidth', 1);
        hold on
        plot3(dim_reduced_data(1, 1), dim_reduced_data(2, 1), dim_reduced_data(3, 1), 'o', 'MarkerFaceColor', color_thisCond, 'MarkerEdgeColor', color_thisCond)
    end
    s_id = s_id+1;
end
...
%% testing
for i = 1:size(allFactors_1, 1)
    plot(allFactors_1(i,:))
    hold on
end

%% testing
for i = 1:size(factor3, 1)
    %for i = 1:size(factor3, 1)
    plot(factor3(i,:))
    hold on
end
...
ind1 = 1;
ind3 = 1;
factor1 = zeros( 602, numBins);
factor3 = zeros( 602, numBins);
for nday = 1:numel(alf)
    trialsToKeepInds{ nday } = find( trialsToKeep{ nday } );
    for itr = 1:numel( trialsToKeepInds{ nday } )
        ntr = trialsToKeepInds{ nday }( itr );
        if UEs{nday}.cueLoc( ntr ) == 1
            factor1(ind1,:) = alf{ nday }( ntr ).rates( 1, alf{ nday }( ntr ).( whichfieldDimred ) + timePoints );
            ind1 = ind1 +1;
        else
            factor3(ind3,:) = alf{ nday }( ntr ).rates( 1, alf{ nday }( ntr ).( whichfieldDimred ) + timePoints );
            ind3 = ind3 + 1;
        end        
    end
end
