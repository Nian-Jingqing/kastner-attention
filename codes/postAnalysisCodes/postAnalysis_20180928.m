...
%%
loadChoppedCombined_fourLocations


%% make a place to store output videos
% outdir = '/Users/feng/SNEL/tmp/kastnervid/';
outdir = '/snel/share/share/derived/kastner/LFADS_runs/pulvinar/Multi-day/multiDay_CO_AO_TD_HoldRel_JanToApr/postAnalysis/withExternalInput_20180928/lowD/tmp/';
if ~isdir( outdir )
    mkdir( outdir );
end
...
for nday = 1 : numel( alf )
    ...
    trialsToKeep{ nday } = isCorrectArray{ nday } & (UEs{ nday }.cueLoc == 1 | UEs{ nday }.cueLoc == 3);
...
%% only do dimred based on day 1
nday = 1;
timePoints = window(1):window(2);
numBins = numel( timePoints);
numFactors = size( alf{ nday }(1).spike_smoothed, 1);
...
for nday = 1
    ...
    for itr = 1:numel( trialsToKeepInds{ nday } )
        ...
        allFactors( :, (0:numBins-1) + ind ) = alf{ nday }( ntr ).spike_smoothed( :, alf{ nday }( ntr ).( whichfieldDimred ) + timePoints );
        ind = ind + numBins;
    end
end
...
%for nt = 1:5:60
    ...
    for nday = 1
        ...
        for itr = 1:numel( trialsToKeepInds{ nday } )
            ...
            frep = alf{ nday }( ntr ).spike_smoothed( :, alf{ nday }( ntr ).( whichfieldPlot ) + window );
            % mean center
    ...
    for np = 1:numel(p)
        ...
        %set(p(np), 'view', [-39.2000   19.6000]); % cp -many videos made using this view
        
        %set(p(np), 'view', [-4.8000   -2.0000]); % fz - view that see oscillation the best (cue3, HRHR)
        set(p(np), 'view', [-6.0000   -90.0000]); % fz - view entering the oscillation the best (also aka, the strong visual response dimension) (cue3, HRHR)
        
        %set(p(np), 'view', [-76.0000   5.2000]); % fz - best oscillation view (cue3, HHRR)
        %set(p(np), 'view', [-5.2000   -45.2000]); % fz - best entering oscillation view (cue3, HHRR)

        %set(p(np), 'view', [-53.2000   13.2000]); % fz - view for smoothed spiking low-d traj
        %set(p(np), 'view', [-106.8000   10.0000]); % fz - view for smoothed spiking - plane view for oscillation period

        %axis(p(np), [ -0.6    0.4    -0.5    0.9    -0.5     0.4]); % this works best for cue3, HHRR
        
        %axis(p(np), [ -0.6    1    -0.5    1    -0.4     0.4]); % this works best for cue3, HRHR
        %axis(p(np), paxis);
        % axis(p(np), [ -1.36    0.4   -0.79    0.63   -1.3615    0.9]);
        %axis(p(np), [-1.5    0.4   -1.10    1.7203   -1.9    0.6567]);% this works well for cueOnset
        axis(p(np), [ -1    1    -0.5    1    -0.5    0.8 ]); % this works well for smoothed spiking low-d traj
