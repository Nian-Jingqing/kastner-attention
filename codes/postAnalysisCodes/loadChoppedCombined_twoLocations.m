...
for nday = 1: numel( datasets )
    ...
    % get the actual lfads firing rate, called FR here.
    disp( sprintf( 'getting LFADS firing ratess day %g / %g', nday, numel( datasets ) ) );
    day_id = nday;
    r_lfads3 = olapChopped{ nday }.r.get_output_from_lfads(run, day_id, trial_time_ms, trial_olap_ms, 'rates');
...
%for i = 1:numel(alf)
%    for j = 1:numel(alf{i})
%        lfadsOut{i}(j).factors = alf{i}(j).rates;
%        lfadsOut{i}(j).rates = alf{i}(j).FR;
%        lfadsOut{i}(j).cueOnset = alf{i}(j).cueOnset;
%        lfadsOut{i}(j).arrayOnset = alf{i}(j).arrayOnset;
%        lfadsOut{i}(j).targetDim = alf{i}(j).arrayDim;
%    end
%end

%%
%whichNeurons{1} = [1 4 6 7 8 11 12 13 14 16 17 19 20 21 23 24 25 26 27 29 30 31 32];
% % 170127
%whichNeurons{2} = [1 2 4 5 6 7 8 9 12 14 15 16 17 19 20 23 24 25 26 29 30 31 32];
% % 170130
%whichNeurons{3} = [1 5 7 8 10 12 13 27 30 32];
% % 170201
%whichNeurons{4} = [3 4 5 7 11 12 15 17 18 23 28 30];
% % 170211
%whichNeurons{5} = [9 15 18 20 24 28 30 31 32 34 37 38 39 42 43 44 45 46 47 48 52 54 55 %58 60 62 63 64];
% % 170308
%whichNeurons{6} = [2 3 5 8 9 11 13 17 20 22 25 26 32 33 36 38 42 43 50 54 55 56 57 58];
% % 170311

%%
%clear data
%data.lfadsOut = lfadsOut;
%data.whichNeurons = whichNeurons;
%saveDir2 = '/snel/share/share/derived/kastner/LFADS_runs/pulvinar/Multi-day/multiDay_CO%_AO_TD_HoldRel_JanToApr/postAnalysis/withExternalInput_20180614/dataForRyan/';
%cd(saveDir2)
%saveName = 'run180614_twoLocs_sixDays_170127to170311';
%save(saveName, 'data');