...
outdir = '/snel/share/share/derived/kastner/LFADS_runs/pulvinar/Multi-day/multiDay_CO_AO_TD_HoldRel_JanToApr/postAnalysis/reRun180614_20190620/jPCA_lowD_traj/tmp/';
if ~isdir( outdir )
    mkdir( outdir );
end
...
%% do pca
meanFactors = mean( allFactors' ); % use factor

[pca_proj_mat, pc_data] = pca( allFactors', 'NumComponents', 10); % use rate
...
for nday = 1:numel( alf )
    ...
    for itr = 1:numel( trialsToKeepInds{ nday } )
        ...
        dataForJPCA(ind).A = dim_reduced_data(1:8,:)';
