...
outdir = '/snel/share/share/derived/kastner/LFADS_runs/pulvinar/Multi-day/multiDay_CO_AO_TD_HoldRel_JanToApr/postAnalysis/withExternalInput_20181207/traj_LowD/arrayDim/jPCA_progression_tmp/noFilter';
if ~isdir( outdir )
    mkdir( outdir );
end
...
for nday = 1 : numel( alf )
    ...
    %    isLongDelay{ nday } = ( [ alf{ nday }.arrayOnset ] - [ alf{ nday }.cueOnset] ) > ( minimalDelay/binsize_rescaled ); % try this later
    %    trialsToKeep{ nday } = isCorrectArray{ nday } & (~[lfp{ nday }.isTrialOutlier])' & ( isLongDelay{ nday } )'; % try this later
    isCueLoc3{ nday } = UEs{ nday }.cueLoc == 3;
    trialsToKeep{ nday } = isCorrectArray{ nday } & isCueLoc3{ nday };% & UE2.isHoldTrial;
...
%% do pca
meanRates = mean( allRates' ); % use rate

[pca_proj_mat, pc_data] = pca( allRates', 'NumComponents', 30); % use rate

%meanFactors = mean( allFactors' ); % use factors

%[pca_proj_mat, pc_data] = pca( allFactors', 'NumComponents', 10); % use factors
...
%for nday = [1 3]
    ...
    for itr = 1:numel( trialsToKeepInds{ nday } )
        ...
        dataForJPCA(ind).A = dim_reduced_data(1:30,:)';
...
for i = 1: numel(dataForJPCA)
    ...
    sigFiltered = bandpassFilter_singleTrial( double(sigToFilter), 8, 3, 100);
