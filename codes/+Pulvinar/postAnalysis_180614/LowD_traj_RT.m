...
for i = 1:length(assembled_lfads.r)
    assembled_lfads.r(i).cueAlign = assembled_lfads.r(i).rates(:, (cueStart_lfads(i)-(90-1)):(cueStart_lfads(i)+125) );
...
%%
rateMatrix = zeros(30,length(assembled_hold_lfads.r));
timeToAnalysis = 60;
for i = 1:length(assembled_hold_lfads.r)
    rateMatrix(:, i) = assembled_hold_lfads.r(i).dimAlign(:, timeToAnalysis);
end
%%
decoded_rt = zeros(size(rtVector_hold));
trainIndices = true(size(rtVector_hold));
for i = 1:length(rtVector_hold)
    trainIndices(i) = false;
    trainMatrix = rateMatrix(:,trainIndices);
    trainRT = rtVector_hold(trainIndices);
    w = trainRT'\trainMatrix';
    decoded_rt(i) = w*rateMatrix(:, i);
    trainIndices(i) = true;
end
%%
f1 = figure;
scatter(rtVector_hold, decoded_rt)
%%


%%################################################################################
%% put hold cueAlign into assembled_hold_lfads
LFADSHold = assembled_lfads.r(UE.isHoldTrial);

%% select time period to analyze
for i = 1: length(LFADSHold)
    LFADSHold(i).forAnalysis = LFADSHold(i).cueAlign(:,91:215);
end

%%
lfads_hold_cueAlign_allTrials = [LFADSHold.forAnalysis];

%% normalize the data
lfads_hold_cueAlign_allTrials_norm = normalize(lfads_hold_cueAlign_allTrials, 'centered');

%% perform PCA
[pca_proj_mat, pc_data] = pca(lfads_hold_cueAlign_allTrials_norm', 'NumComponents', 3);
dim_reduced_data = pca_proj_mat' * lfads_hold_cueAlign_allTrials_norm;

%% put dim_reduced data back to r struct
startThisTrial = 1;
endThisTrial = 125;
for i = 1:length(LFADSHold)
    LFADSHold(i).dim_reduced_data = dim_reduced_data(:, startThisTrial:endThisTrial);
    startThisTrial = endThisTrial + 1;
    endThisTrial = endThisTrial + 125;
end

%% set up clVector for hold trials for cueAlign hold trials
clVector_cueHold = clVector(UE.isHoldTrial);
cl1_cue = clVector_cueHold == 1;
cl3_cue = clVector_cueHold == 3;

%% plot low-d structure

cl1Trials_cue = LFADSHold(cl1_cue);
cl3Trials_cue = LFADSHold(cl3_cue);
f9 = figure;
for i = 1: length(cl1Trials_cue)
    plot3(cl1Trials_cue(i).dim_reduced_data(1,:), cl1Trials_cue(i).dim_reduced_data(2,:), cl1Trials_cue(i).dim_reduced_data(3,:), 'r-s', 'MarkerIndices', [1, 25, 50, 75, 100, 125], 'markerfacecolor', [0 0 0]);
    hold on
end
for i = 1: length(cl3Trials_cue)
    plot3(cl3Trials_cue(i).dim_reduced_data(1,:), cl3Trials_cue(i).dim_reduced_data(2,:), cl3Trials_cue(i).dim_reduced_data(3,:), '-gs', 'MarkerIndices', 1:10:125, 'markerfacecolor', ...
          [0 0 0]); %[1, 25, 50, 75, 100, 125]
    hold on
end

%%######################## array vs RT ##############################################
%% pick out trials so that the trial # is the same as dimAlign
LFADSHold = assembled_lfads.r(UE.isHoldTrial);
LFADSHold = LFADSHold((stopTimes - dimStart) > 302);

%% select time period to analyze
for i = 1: length(LFADSHold)
    LFADSHold(i).forAnalysis = LFADSHold(i).arrayAlign(:,101:150);
end

%%
lfads_hold_arrayAlign_allTrials = [LFADSHold.forAnalysis];

%% normalize the data
lfads_hold_arrayAlign_allTrials_norm = normalize(lfads_hold_arrayAlign_allTrials, 'centered');

%% perform PCA
[pca_proj_mat, pc_data] = pca(lfads_hold_arrayAlign_allTrials_norm', 'NumComponents', 3);
dim_reduced_data = pca_proj_mat' * lfads_hold_arrayAlign_allTrials_norm;
...
for i = 1:length(LFADSHold)
    LFADSHold(i).dim_reduced_data = dim_reduced_data(:, startThisTrial:endThisTrial);
    startThisTrial = endThisTrial + 1;
    endThisTrial = endThisTrial + 50;
end

%% plot low-d structure
slowTrials = LFADSHold(rtTagSlow);
fastTrials = LFADSHold(rtTagFast);
cl1Trials = LFADSHold(cl1);
cl3Trials = LFADSHold(cl3);
f9 = figure;
for i = 1: length(cl1Trials)
    plot3(cl1Trials(i).dim_reduced_data(1,:), cl1Trials(i).dim_reduced_data(2,:), cl1Trials(i).dim_reduced_data(3,:), '-mo', 'MarkerIndices', [1]);
    hold on
end
for i = 1: length(cl3Trials)
    plot3(cl3Trials(i).dim_reduced_data(1,:), cl3Trials(i).dim_reduced_data(2,:), cl3Trials(i).dim_reduced_data(3,:), '-bo', 'MarkerIndices', [1]);
    hold on
end
xlabel('1st Dimension');
ylabel('2nd Dimension');
zlabel('3rd Dimension');
title('ArrayOn + (100ms - 300ms); cueLoc1 vs cueLoc3');
savedir = '/snel/share/share/derived/kastner/LFADS_runs/pulvinar/Multi-day/multiDay_CO_AO_TD_HoldRel_JanToApr/postAnalysis/withExternalInput_20180614/170311';

% ##############################################################################

%% select time period to analyze
for i = 1: length(assembled_hold_lfads.r)
    assembled_hold_lfads.r(i).forAnalysis = assembled_hold_lfads.r(i).dimAlign(:,1:75);
end

%%
lfads_hold_dimAlign_allTrials = [assembled_hold_lfads.r.forAnalysis];

%% normalize the data
lfads_hold_dimAlign_allTrials_norm = normalize(lfads_hold_dimAlign_allTrials, 'centered');

%% perform PCA
[pca_proj_mat, pc_data] = pca(lfads_hold_dimAlign_allTrials_norm', 'NumComponents', 3);
dim_reduced_data = pca_proj_mat' * lfads_hold_dimAlign_allTrials_norm;

%% put dim_reduced data back to r struct
startThisTrial = 1;
endThisTrial = 75;
for i = 1:length(assembled_hold_lfads.r)
    assembled_hold_lfads.r(i).dim_reduced_data = dim_reduced_data(:, startThisTrial:endThisTrial);
    startThisTrial = endThisTrial + 1;
    endThisTrial = endThisTrial + 75;
end

%% plot low-d structure
slowTrials = assembled_hold_lfads.r(rtTagSlow);
fastTrials = assembled_hold_lfads.r(rtTagFast);
cl1Trials = assembled_hold_lfads.r(cl1);
cl3Trials = assembled_hold_lfads.r(cl3);
f7 = figure;
for i = 1: length(fastTrials)
    plot3(fastTrials(i).dim_reduced_data(1,:), fastTrials(i).dim_reduced_data(2,:), fastTrials(i).dim_reduced_data(3,:), '-ro', 'MarkerIndices', [1]);
    hold on
end
for i = 1: length(slowTrials)
    plot3(slowTrials(i).dim_reduced_data(1,:), slowTrials(i).dim_reduced_data(2,:), slowTrials(i).dim_reduced_data(3,:), '-go', 'MarkerIndices', [1]);
    hold on
end
plot3(assembled_hold_lfads.r(307).dim_reduced_data(1,:), assembled_hold_lfads.r(307).dim_reduced_data(2,:), assembled_hold_lfads.r(307).dim_reduced_data(3,:), '-bo', 'MarkerIndices', [1]);
plot3(assembled_hold_lfads.r(421).dim_reduced_data(1,:), assembled_hold_lfads.r(421).dim_reduced_data(2,:), assembled_hold_lfads.r(421).dim_reduced_data(3,:), '-bo', 'MarkerIndices', [1]);
plot3(assembled_hold_lfads.r(252).dim_reduced_data(1,:), assembled_hold_lfads.r(252).dim_reduced_data(2,:), assembled_hold_lfads.r(252).dim_reduced_data(3,:), '-bo', 'MarkerIndices', [1]);
plot3(assembled_hold_lfads.r(315).dim_reduced_data(1,:), assembled_hold_lfads.r(315).dim_reduced_data(2,:), assembled_hold_lfads.r(315).dim_reduced_data(3,:), '-bo', 'MarkerIndices', [1]);
plot3(assembled_hold_lfads.r(35).dim_reduced_data(1,:), assembled_hold_lfads.r(35).dim_reduced_data(2,:), assembled_hold_lfads.r(35).dim_reduced_data(3,:), '-mo', 'MarkerIndices', [1]);
plot3(assembled_hold_lfads.r(132).dim_reduced_data(1,:), assembled_hold_lfads.r(132).dim_reduced_data(2,:), assembled_hold_lfads.r(132).dim_reduced_data(3,:), '-mo', 'MarkerIndices', [1]);
xlabel('1st Dimension');
ylabel('2nd Dimension');
zlabel('3rd Dimension');
title('(300ms - 0ms) prior to TargetDim; slow vs fast');
%%

% define timeLags that you want to analyze a
timeLagAll = -280:20:-240;
% define sigma for smoothing real spiking data
sigma = 50;
% define the savedir to store the plots
savedir = '/snel/share/share/derived/kastner/LFADS_runs/pulvinar/Multi-day/multiDay_CO_AO_TD_HoldRel_JanToApr/postAnalysis/withExternalInput_20180614/tSNE/170311';
% #############################################
