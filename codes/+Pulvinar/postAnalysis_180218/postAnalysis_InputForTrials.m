...
%%
savedirOne = ['/snel/share/share/derived/kastner/LFADS_runs/pulvinar/Multi-day/multiDay_CO_AO_TD_HoldRel_JanToMar' ...
    '/postAnalysis/withGoodNeurons_Run_20180218/PlottingInputsPatchline/'];
f1 = figure;
clear set
...
for i = 1:nInputs
    ...
    sp((i-1)*3 + 1) = subplot(6, 3, (i-1)*3 + 1);
    c1_Input = condStruct(1).inputTensor(:, chopStart:chopEnd,i);
    x_value = 1:size(c1_Input,2);
    for t = 1:size(c1_Input,1)
        Plot.patchline(x_value,c1_Input(t,:),'edgecolor','r','linewidth',1,'edgealpha',0.5);
        hold on
    end
 
    
    c2_Input = condStruct(2).inputTensor(:,chopStart:chopEnd,i);
    for t = 1:size(c2_Input,1)
        Plot.patchline(x_value,c2_Input(t,:),'edgecolor','b','linewidth',1,'edgealpha',0.5);
        hold on
    end
    set(gca,'XTick',[1 div*0.5*nTimesLFADS div*nTimesLFADS]);
    set(gca,'XTickLabels',{'-800','CueOnset','+800'});
    xlim([0 div*nTimesLFADS]);
    title(sp((i-1)*3 + 1), ['CueOnset - Input ' int2str(i)]);
    hold off
    
    
    
    sp((i-1)*3 + 2) = subplot(6, 3, (i-1)*3 + 2);
    c3_Input = condStruct(3).inputTensor(:, chopStart:chopEnd,i);
    for t = 1:size(c3_Input,1)
        Plot.patchline(x_value,c3_Input(t,:),'edgecolor','r','linewidth',1,'edgealpha',0.5);
        hold on
    end
    c4_Input = condStruct(4).inputTensor(:,chopStart:chopEnd,i);
    for t = 1:size(c4_Input,1)
        Plot.patchline(x_value,c4_Input(t,:),'edgecolor','b','linewidth',1,'edgealpha',0.5);
        hold on
    end
    set(gca,'XTick',[1 div*0.5*nTimesLFADS div*nTimesLFADS]);
    set(gca,'XTickLabels',{'-800','ArrayOnset','+800'});
    xlim([0 div*nTimesLFADS]);
    title(sp((i-1)*3 + 2), ['ArrayOnset - Input ' int2str(i)]);
    hold off
    
    
    
    sp((i-1)*3 + 3) = subplot(6, 3, (i-1)*3 + 3);
    c5_Input = condStruct(5).inputTensor(:, chopStart:chopEnd,i);
    h1(i) = shadedErrorBar([], c5_Input, {@mean, @(x) std(x)./sqrt(size(c5_Input, 1)) }, 'lineProps', '-r');
    hold on
    c6_Input = condStruct(6).inputTensor(:,chopStart:chopEnd,i);
    h2(i) = shadedErrorBar([], c6_Input, {@mean, @(x) std(x)./sqrt(size(c6_Input, 1)) }, 'lineProps', '-b');
    set(gca,'XTick',[1 div*0.5*nTimesLFADS div*nTimesLFADS]);
    set(gca,'XTickLabels',{'-800','TargetDim','+800'});
    xlim([0 div*nTimesLFADS]);
    title(sp((i-1)*3 + 3), ['TargetDim - Input ' int2str(i)]);
