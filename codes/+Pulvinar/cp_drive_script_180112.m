...
addpath('/snel/home/cpandar/Feng_tmp/Pulvinar/codes')
addpath('/snel/home/cpandar/analysis_tools')
datasetPath = ['/snel/home/cpandar/Feng_tmp/Pulvinar/M20170608_PUL_all-g2-g3-g4-evokedSpiking-v7/' ...
...
for r_id = length(dc.datasets)
    disp( sprintf( 'loading %g', r_id ) );
...
for nLag = 1:length(timeLagAll)
    ...
    title(['cueOnset ' num2str(timeLag*4, '%+0.0f')]);
    keyboard
%     print(f5,['cueOnset ' num2str(timeLag*4, '%+0.0f')], '-dpng');
...
nRates = size( lfadsAlign(1).rates, 1 );
timesToGet =  80;%-40:40:320;
indicesToGet = timesToGet / 4;
nTimes = numel( timesToGet );
tsneMatrix = zeros( nTimes * nTrials, nRates );

tsneIndex = 0;
indexOffset = 200;
for itimes = 1:nTimes
    thisIndex = indicesToGet( itimes ) + indexOffset;
    for itrial = 1:nTrials
        tsneIndex = tsneIndex + 1;
        tsneMatrix( tsneIndex, : ) = lfadsAlign( itrial ).rates(:, thisIndex);
    end
end
    
%% actually run tsne
Rate_tsne = tsne( tsneMatrix );
...
for itimes = 1:nTimes
    f5 = figure(1);
    clf;
    ratesThisTime = Rate_tsne( (1:nTrials) + ( itimes-1 ) * nTrials, :);
    
    for itrial = 1:nTrials
        h( itrial ) = scatter(ratesThisTime( itrial, 1), ratesThisTime(itrial, 2));
        hold on
        set(h( itrial ), 'markerfacecolor', 'none');
        set(h( itrial ), 'markeredgecolor',trialColor( clVector( itrial ), : ) );
        set(h( itrial ), 'Marker', trialShape( clVector( itrial ) ) );
        % use the cued Location value in clVector to index to the proper trial
        % shape in trialShape vector
    end
