...
for r_id = 1:length(rc.runs)
    ...
    for nData = 1:length(run.sequenceData)
        r_lfads(nData) = R.Rstruct(run.sequenceData{nData});
    end
%     r_lfads(r_id) = R.Rstruct(run.sequenceData{1}); % Put sequence data into a struct
end

%% Select the day you want to analyse
r_realCopy = r_real(2).copy();
...
savedirOne = ['/snel/share/share/derived/kastner/LFADS_runs/pulvinar/Multi-day/multiDay_CO_AO_TD_HoldRel_JanToMar' ...
    '/postAnalysis/withGoodNeurons_Run_20180211/PSTH/Gaussian10ms/cueOnset/170130'];
...
%     set(gca,'YLim',sPlot_avgRealRates.YLim);
    xlabel('time (ms)');
    Legend = legend('show');
    set(Legend,'Position',[0.7802 0.5313 0.1226 0.0425]);
