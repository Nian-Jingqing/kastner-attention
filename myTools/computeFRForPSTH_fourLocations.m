function [AvgFiringRate_InRF,AvgFiringRate_OffRF, AvgFiringRate_PlusRF, AvgFiringRate_MinusRF, SmoothedSpikingMatrix, TrialIndexInRF, TrialIndexOffRF, TrialIndexPlusRF, TrialIndexMinusRF,  nTimesLFADS] = computeFRForPSTH_fourLocations(assembled, infield, sigma, binsize, olapChopped, type)
...
AvgFiringRate_PlusRF = zeros(nNeurons, nTimesLFADS);
AvgFiringRate_MinusRF = zeros(nNeurons, nTimesLFADS);
...
TrialIndexPlusRF = false(nNeurons, nTrials);
TrialIndexMinusRF = false(nNeurons, nTrials);
...
%     AvgLFADSRate(n,:) = (sum(WholeLFADSRatesMatrix(:,:,n),1))*(1/nTrials);
    ...
    TrialIndexPlusRF(n,:) = clVector == cycleIndex(rfLoc(n,1) + 1);
    nPlusRF = length(clVector(clVector == cycleIndex(rfLoc(n,1) + 1)));
    TrialIndexMinusRF(n,:) = clVector == cycleIndex(rfLoc(n,1) - 1);
    nMinusRF = length(clVector(clVector == cycleIndex(rfLoc(n,1) - 1)));
...
%     % find the number of trials that cue loc is OutRF 2
    if strcmp(type, 'lfads')
        ...
        AvgFiringRate_PlusRF(n,:) = (sum(SmoothedSpikingMatrix(TrialIndexPlusRF(n,:),:,n),1))*(1/nPlusRF);
        AvgFiringRate_MinusRF(n,:) = (sum(SmoothedSpikingMatrix(TrialIndexMinusRF(n,:),:,n),1))*(1/nMinusRF);
    else
        ...
        AvgFiringRate_PlusRF(n,:) = (sum(SmoothedSpikingMatrix(TrialIndexPlusRF(n,:),:,n),1))*(1/nPlusRF)*(1000/rebinSize);
        AvgFiringRate_MinusRF(n,:) = (sum(SmoothedSpikingMatrix(TrialIndexMinusRF(n,:),:,n),1))*(1/nMinusRF)*(1000/rebinSize);
    end
