...
% @ 2009 Byron Yu -- byronyu@stanford.edu
  ...
  kernSD = [];
  GPFA.Util.assignopts(who, varargin);
  ...
  if ismember(ws.method, {'gpfa'})
    C               = ws.estParams.C;
    X               = [ws.seqTrain.xsm];    
    [Xorth, Corth]  = GPFA.Util.orthogonalize(X, C); 
    seqTrain        = GPFA.Util.segmentByTrial(ws.seqTrain, Xorth, 'xorth');
    ...
    if ~isempty(ws.seqTest)
      ...
      ws.seqTest     = GPFA.Core_GPFA.exactInferenceWithLL(ws.seqTest, estParams);
      X              = [ws.seqTest.xsm];
      [Xorth, Corth] = GPFA.Util.orthogonalize(X, C);
      seqTest        = GPFA.Util.segmentByTrial(ws.seqTest, Xorth, 'xorth');            
    end
  
  elseif ismember(ws.method, {'fa', 'ppca'})
    L               = ws.kern(k).estParams.L;
    X               = [ws.kern(k).seqTrain.xpost];
    [Xorth, Lorth]  = GPFA.Util.orthogonalize(X, L); 
    seqTrain        = GPFA.Util.segmentByTrial(ws.kern(k).seqTrain, Xorth, 'xorth');    
    ...
    if ~isempty(ws.kern(k).seqTest)
      ...
      X              = GPFA.Core_twostage.fastfa_estep(Y, ws.kern(k).estParams);
      [Xorth, Lorth] = GPFA.Util.orthogonalize(X.mean, L);
      seqTest        = GPFA.Util.segmentByTrial(ws.kern(k).seqTest, Xorth, 'xorth');    
    end
        
  elseif ismember(ws.method, {'pca'})
    % PCA is already orthonormalized
    X               = [ws.kern(k).seqTrain.xpost];
    seqTrain        = GPFA.Util.segmentByTrial(ws.kern(k).seqTrain, X, 'xorth');
