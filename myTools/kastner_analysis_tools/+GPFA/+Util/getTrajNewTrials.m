...
% Aug 21, 2011: Added option for causal smoothing
  ...
  seqNew  = GPFA.Util.getSeq(dat, ws.binWidth, ws.extraOpts{:});
  ...
  if isfield(ws, 'kernSDList')
    ...
    % Kernel smoothing
    for n = 1:length(seqNew)
      seqNew(n).y = GPFA.Util.smoother(seqNew(n).y, ws.kernSDList(k), ws.binWidth, ...
			     'causal', causal);
    end
  end

  if ismember(ws.method, {'gpfa'}) & ~causal
    seqNew         = GPFA.Core_GPFA.exactInferenceWithLL(seqNew, ws.estParams);
    C              = ws.estParams.C;
    X              = [seqNew.xsm];
    [Xorth, Corth] = GPFA.Util.orthogonalize(X, C);
    seqNew         = GPFA.Util.segmentByTrial(seqNew, Xorth, 'xorth');

  elseif ismember(ws.method, {'gpfa'}) & causal
    seqNew         = GPFA.Core_GPFA.causalInference(seqNew, ws.estParams, extraOpts{:});
    C              = ws.estParams.C;
    X              = [seqNew.xfi];
    [Xorth, Corth] = GPFA.Util.orthogonalize(X, C);
    seqNew         = GPFA.Util.segmentByTrial(seqNew, Xorth, 'xorth');
