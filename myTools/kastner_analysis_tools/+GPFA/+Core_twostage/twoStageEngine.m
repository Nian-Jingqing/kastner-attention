...
% @ 2009 Byron Yu -- byronyu@stanford.edu
  ...
  for k = 1:length(kernSDList)
    ...
    for n = 1:length(seqTrain)
      kern(k).seqTrain(n).y = GPFA.Util.smoother(seqTrain(n).y, kernSDList(k), binWidth);    
    end
    
    % Test data
    kern(k).seqTest = seqTest;
    for n = 1:length(seqTest)
      kern(k).seqTest(n).y = GPFA.Util.smoother(seqTest(n).y, kernSDList(k), binWidth);
    end
    ...
    if ismember(typ, {'fa', 'ppca'})
      [estParams, LL] = GPFA.Core_twostage.fastfa(Y, xDim, 'typ', typ, extraOpts{:});
      X               = GPFA.Core_twostage.fastfa_estep(Y, estParams);
      ...
      kern(k).seqTrain  = GPFA.Util.segmentByTrial(kern(k).seqTrain, X.mean, 'xpost');
    ...
    elseif isequal(typ, 'pca')
      ...
      kern(k).seqTrain = GPFA.Util.segmentByTrial(kern(k).seqTrain,... 
    ...
    if ~isempty(seqTest) % check if there are any test trials
      ...
      if ismember(typ, {'fa', 'ppca'})
        Ycs             = GPFA.Core_twostage.cosmoother_fa(Y, kern(k).estParams);
        kern(k).seqTest = GPFA.Util.segmentByTrial(kern(k).seqTest, Ycs, 'ycs');
        
      elseif isequal(typ, 'pca')
        Ycs             = GPFA.Core_twostage.cosmoother_pca(Y, kern(k).estParams);      
        kern(k).seqTest = GPFA.Util.segmentByTrial(kern(k).seqTest, Ycs, 'ycs');    
      end
    end
  end
