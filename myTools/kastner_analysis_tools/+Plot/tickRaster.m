function hLine = tickRaster(timesCell, varargin)
    ...
    if ~isempty(p.Results.waveCell) && ~p.Results.quick
        ...
        tvec = Utils.makerow(p.Results.waveformTimeRelative * p.Results.waveScaleTime);
    ...
    elseif ~p.Results.quick
        ...
        for iE = 1:nTrials 
            if ~isempty(timesCell{iE})
                XByTrial{iE} = [repmat(Utils.makerow(timesCell{iE}), 2, 1); nan(1, numel(timesCell{iE}))];
