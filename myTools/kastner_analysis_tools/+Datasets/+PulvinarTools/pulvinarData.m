classdef pulvinarData < R.Rstruct   
    methods
        % constructor
        function obj = pulvinarData( varargin )
        ...
        function [ data ] = generate_overlap_chop_lfads_data( r, trial_time_ms, trial_olap_ms )
            outStruct(length(r.r)).counts = 0;
            for r_trial = 1:length(r.r)
                spikes = r.r(r_trial).spikes;
                data_time_ms = size( spikes, 2 );
                % determine number of channels
                n_dim = size( spikes, 1 );
                % determine number of trials
                n_trials = ceil( ( data_time_ms - trial_time_ms ) / ( trial_time_ms - trial_olap_ms ) );
                ...
                % get inputs in continous form
                inputs = r.r(r_trial).externalInputs;
                % get length of data in ms (pretty easy since 1ms binned data)
                data_time_ms = size( inputs, 2 );
                % determine number of channels
                n_dim = size( inputs, 1 );
                % determine number of trials
                n_trials = ceil( ( data_time_ms - trial_time_ms ) / ( trial_time_ms - trial_olap_ms ) );
                ...
                states = zeros( n_trials, n_dim, trial_time_ms );
                % now generate input states
                for i_trial=1:n_trials
                    % get start idx (ms)
                    start_idx = ( i_trial-1 )*trial_time_ms - ( i_trial-1 )*trial_olap_ms + 1;
                    % get end idx( ms)
                    ...
                    % try to extract trial
                    try
                        states( i_trial, :, : ) = inputs( :, start_idx:end_idx );
                    % but if you go over index limit of matrix
                    catch ME
                        % start from the end of the data and have more overlap for the last trial
                        start_idx = data_time_ms - trial_time_ms + 1;
                ...
                outStruct(r_trial).counts = permute(counts, [2 3 1]);
                outStruct(r_trial).externalInputs = permute(states, [2 3 1]);


%                 % intialize output struct
%                 data = struct();
%                 data.counts = counts;
%                 data.externalInputs = states;
%                 data.timeVecMs = 1:trial_time_ms;
%                 data.conditionId = 1:n_trials;
            end
            data = struct();
            data.counts = permute(cat(3, outStruct.counts), [3 1 2]);
            data.externalInputs = permute(cat(3, outStruct.externalInputs), [3 1 2]);
            data.timeVecMs = 1:trial_time_ms;
            data.conditionId = 1:size(data.counts,1);
        end
%############################# Lahiru's chop-overlap method (leave 0s at the end of a trial)
%         function [ data ] = generate_overlap_chop_lfads_data( r, trial_time_ms, trial_olap_ms )
%             
...
%             data.conditionId = 1:n_trials;
%         end
        function[ r_lfads ] = get_output_from_lfads( r, run, dayId, trial_time_ms_preBin, trial_olap_ms_preBin, ratesOrFactors )
...
%             end
            ...
            if strcmp(ratesOrFactors, 'rates')
            tot_rates = run.posteriorMeans(dayId, 1).rates;
            elseif strcmp(ratesOrFactors, 'factors')
            tot_rates = run.posteriorMeans(dayId, 1).factors;
            end
...
%             tot_factors = permute( tot_factors, [ 3 1 2 ] );
            ...
            r_lfads(length(r.r)).rates = 1;
            binWidthMs = run.params.spikeBinMs;
            trial_time_ms = trial_time_ms_preBin/binWidthMs;
            trial_olap_ms = trial_olap_ms_preBin/binWidthMs;
            
            % Design filters
            lfadsFilter1 = r.designTrialFilter( trial_time_ms, trial_olap_ms, 0 );
            lfadsFilter2 = r.designTrialFilter( trial_time_ms, trial_olap_ms, 1 );
            lfadsFilter3 = r.designTrialFilter( trial_time_ms, trial_olap_ms, 2 );
            ...
            for r_trial = 1: length(r.r)
                ...
                for i = 1:nPieces
                    ...
                    ratesThisPiece = r.filterChannel( ratesThisPiece, lfadsTrialFilter);
                    
                    trialStart = 1 + (i-1)*trial_time_ms - (i-1)*trial_olap_ms;
                    trialEnd = trialStart + trial_time_ms - 1;
                    r_lfads(r_trial).rates( :, trialStart:trialEnd ) = r_lfads(r_trial).rates( :, trialStart:trialEnd ) + ratesThisPiece;
                end
        ...
        function [ filt_main ] = designTrialFilter( r, trialTime, trialOlap, filterType )
